...
int PolyHeadElement(PPosition newPoly);
...
//treba one sortove po velicini i da se zbrajaju i to za pojedinacne polinome
//konacni zbroj i mnozenje
...
int CreatePoly(PPosition pHead, char* buffer) {
	...
	while (strlen(buffer) > 0) {
		if ((sscanf(buffer, "%d %d %n", &coef, &exp, &bytes)) == 2) {
			if(coef!=0)													//filter
